...
import (
	...
	"data-platform-api-kanban-container-deletes-rmq-kube/config"
...
func (c *DPFMAPICaller) headerDelete(
	...
	header.KanbanContainer = input.Header.KanbanContainer
	header.IsMarkedForDeletion = input.Header.IsMarkedForDeletion
	res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": header, "function": "KanbanContainerHeader", "runtime_session_id": sessionID})
	if err != nil {
